...
image::konzept/around-aspect-ablauf.dn.svg[]
...
Hierüber lassen sich die xref:konzept/integration-isyfact.adoc[integrierten IsyFact-Bausteine] und der Task selbst konfigurieren.
...
Hier springt der Baustein Task Scheduling ein und ermöglicht es, Tasks genau einmal auszuführen, ohne die Vorteile der xref:konzept/integration-isyfact.adoc[] zu verlieren.
