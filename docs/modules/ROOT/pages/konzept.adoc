...
Tasks sind in diesem Kontext periodisch wiederkehrende Tätigkeiten, die innerhalb einer xref:isyfact-standards-doku:referenzarchitektur:fachlich.adoc#awl-fachliche-architektur[Anwendungskomponente] angesiedelt sind und zur Erfüllung der fachlichen oder technischen Anforderungen der Anwendung beitragen.
Sie <<abgrenzung-aufgaben-batches,grenzen sich wesentlich>> von xref:glossary::terms-definitions.adoc#glossar-batch[Batches] ab.
...
image::konzept/bausteinkontext.dn.svg[]
...
Wie der Baustein diese Produkte nutzt, beschreibt die Seite "xref:konzept/erweiterung-spring.adoc[]".

Des Weiteren nutzt er mehrere Bausteine der IsyFact, um ihre Funktion für Tasks verfügbar zu machen.
Dazu gehören die Bausteine xref:isyfact-standards-doku:isy-logging:konzept/master.adoc[Logging], xref:4.0.0@isyfact-standards-doku:isy-security:konzept/master.adoc[Security] und xref:isyfact-standards-doku:isy-ueberwachung:konzept/master.adoc[Überwachung].
Wie genau die Integration aussieht, beschreibt die Seite "xref:konzept/integration-isyfact.adoc[]".
