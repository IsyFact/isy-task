...
Die Details einer `ClientRegistration`-Konfiguration sind in den xref:security::nutzungsvorgaben.adoc#authentifizierungsmanager-authentication[Nutzungsvorgaben Sicherheit und der Authentifizierung eines OAuth 2.0 Clients mit dem Authentifizierungsmanager] beschrieben.
Hierbei kann der Sicherheitsbaustein Client Credentials verwenden.
Die Verwendung von Client Credentials Flow ermöglicht die Authentifizierung und Autorisierung der Anwendung selbst, um auf geschützte Ressourcen zuzugreifen.
