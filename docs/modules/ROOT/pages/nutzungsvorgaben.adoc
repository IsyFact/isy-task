...
Die Details einer `ClientRegistration`-Konfiguration sind in den xref:4.0.0@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc#authentifizierungsmanager-authentication[Nutzungsvorgaben Sicherheit und der Authentifizierung eines OAuth 2.0 Clients mit dem Authentifizierungsmanager] beschrieben.
